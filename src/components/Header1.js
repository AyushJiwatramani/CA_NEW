...
const Header1 = ({ className = "" }) => {
  ...
  return (
    <div
      className={`self-stretch flex-1 flex flex-col items-start justify-start gap-[4px] max-w-full ${className}`}
    >
      <header className="self-stretch flex-1 flex flex-row items-start justify-between py-[30px] pr-[29px] pl-[45px] box-border relative max-w-full gap-[20px] text-center text-sm text-light-mode-white-5-ffffff font-button-2 mq750:pl-[22px] mq750:box-border">
        ...
        <div className="self-stretch flex flex-row items-start justify-start gap-[8px] z-[1] text-gray-200 mq750:hidden">
          <div
            className="rounded-71xl bg-light-mode-white-5-ffffff flex flex-row items-start justify-start py-3 px-[15px] whitespace-nowrap cursor-pointer"
          ...
          <Button
            className="self-stretch w-[68px] cursor-pointer"
            disableElevation
            variant="outlined"
            sx={{
              textTransform: "none",
